...
- Receita específica do livro você não sugere por aqui: as receitas ficam na área de membros, já filtradas pras restrições dela. Pode falar de comida em geral à vontade.
...
- "o que como hoje/amanhã", "meu plano", "o que tem no almoço" → plano_do_dia.
...
const FERRAMENTAS = [
  ...
  { type: 'function', function: { name: 'plano_do_dia', description: 'Mostra as refeições do plano da Nutri Luciana pra hoje ou amanhã.', parameters: { type: 'object', properties: { dia: { type: 'string', enum: ['hoje', 'amanha'] }, so_agora: { type: 'boolean', description: 'true pra mostrar só a refeição desta hora' } }, required: ['dia'], additionalProperties: false } } },
...
export async function contextoDaConversa(contato) {
  ...
  if (t.kcal) {
    ...
    if (meals.length) linhas.push(`Plano de hoje: ${meals.map((m) => `${ROTULO[m.slot] || m.slot} ${m.time || ''} ${bonito(m.name)} (${n0(m.kcal)} kcal)`).join(' · ')}.`);
  ...
  if (dia.entries.length) linhas.push(`Registrado hoje: ${dia.entries.map((e) => `${ROTULO[e.slot]}: ${(e.items || []).map((i) => i.name).join(', ')}`).join(' · ')}.`);
